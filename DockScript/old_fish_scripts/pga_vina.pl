...
sub do_task{
	...
	return undef if!((my $run_time=run_gaVina($argv)) >0);
	#getLowConform($argv->[8]);
	print "Ga_vina Dock $lig into $argv->[0] COMPLETE in:$run_time"."s [by LCZ]\n";
	return 0;
}
...
sub parseParam{
	...
	push(@argv,"$ENV{PBS_O_WORKDIR}/$ENV{lig}");	#配体文件使用全路径
	...
	push(@argv,"$arr[0]/$arr[1]/");	#vina的工作路径 PDBID/LIGID/
	return \@argv;
}
...
sub run_gaVina{
	...
	#print "enter run\n";
	my $ini_t=time();
	my $cmd="/yp_home/licz/bin/Vina/ga_vina  --receptor $argv->[0] --ligand $argv->[1] ";
	$cmd.=	"--center_x $argv->[2] --center_y $argv->[3] --center_z $argv->[4] ";
	$cmd.=	"--size_x $argv->[5] --size_y $argv->[6] --size_z $argv->[7] ";
	$cmd.=	"--out $argv->[8]\_ga.pdbqt";
	if(!open GA,'-|',"$cmd 2>&1"){
		print STDERR "Fail to launch ga_vina for $argv->[8]!\n";
		return 0;
	}
	while(<GA>){
		if (/^ERROR/){
	...
	$end_t -= $ini_t;
	if($err_buf){
		open(ERR,">$argv->[8]\_ga.err");
		print ERR $err_buf;
		close ERR;
		print STDERR "Fail to dock $argv->[8] into $argv->[0] by Ga_vina\n";
		return 0-$end_t;		#如果成功启动了ga_vina但是在中途出现问题导致失败
	}
	if($bindEnergy[0]){
		open(SCORE,">$argv->[8]\_ga.score");
		foreach(@bindEnergy){
			print SCORE $_."\n";
		}
		close SCORE;

...
sub store_res{
	...
	$this->get_res('vote');
		...
		my @data_set;
		my $i;
		open(PBSA,"<$this->{ligand}.vote");
		while(<PBSA>){
			my @arr = split;
			if($arr[2] ne 'null'){
				my $a =shift @arr;
				my $b =shift @arr;
				my $data = "('$this->{prot}->{$a}','$a','$b',";
				$data .= $this->{vote}->{"$a $b"};
				#$data .= ','.$this->{mc}->{"$a $b"};
				#$data .= ','.$this->{le}->{"$a $b"};
				#$data .= ','.$this->{pso}->{"$a $b"};
				#$data .= ','.$this->{plt}->{"$a $b"} ;
				$data .= ",'$a\_$b\_$this->{ligand}_le.pdb')";
				push(@data_set,$data);
			}
			if(++$i >= 100){
				my $sql = "INSERT INTO $db.reversedock_$this->{job_id} (protID,pdb,site,dock_score,file_path) VALUES" . join(',',@data_set);
				sqlExec($sql);
				$i = 0;			#计数器归零
				@data_set = ();	#清空数据集
			}

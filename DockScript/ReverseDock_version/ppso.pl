...
sub Main{
	...
	Log("Starting PSO docking for $lig_name");
	...
	Log("Ending PSO docking for $lig_name");
	return 0;
}
exit(Main);
#==sub rout====================================##
...
sub parseParam{
	...
	my @argv;
	push(@argv,"$path/$arr[0]/$arr[0].pdbqt");	#受体文件
	push(@argv,"$path/$lig_name.pdbqt");		#配体文件
	...
	push(@argv,"$path/$arr[0]/$arr[1]/$lig_name\_pso");	#输出文件名
...
sub run{
	...
	#print "$cmd\n";
	if(!open PSO,'-|',"$cmd 2>$argv->[8].err"){
		print STDERR "Error occured during the PSO docking of $argv->[8]!\n";
		return undef;
	}
	while(<PSO>){
		if(/^   \d/o){

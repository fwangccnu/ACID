...
if mini_option == 'YES':
	...
	min3()
	os.system('mpirun -np 2 sander.MPI -O -i min1.in -o min1.out -p complex.top -c complex.crd -r min1.rst -ref complex.crd') #Movements was allowed only for the water molecules, ions and Hydrogens
	os.system('mpirun -np 2 sander.MPI -O -i min2.in -o min2.out -p complex.top -c min1.rst -r min2.rst -ref min1.rst') #The backbone atoms were fixed, and other atoms were allowed to move
	os.system('mpirun -np 2 sander.MPI -O -i min3.in -o min3.out -p complex.top -c min2.rst -r min3.rst -ref min2.rst') #Minimaze all atoms
	#os.system('sander -O -i min1.in -o min1.out -p complex.top -c complex.crd -r min1.rst -ref complex.crd') #Movements was allowed only for the water molecules, ions and Hydrogens
	#os.system('sander -O -i min2.in -o min2.out -p complex.top -c min1.rst -r min2.rst -ref min1.rst') #The backbone atoms were fixed, and other atoms were allowed to move
	#os.system('sander -O -i min3.in -o min3.out -p complex.top -c min2.rst -r min3.rst -ref min2.rst') #Minimaze all atoms
	#print "optimize 3 finished",time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time()))
...
try:
	#print "starting mmpbsa",time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time()))
        os.system('mm_pbsa.pl mm_pbsa.in > mm_pbsa.log 2>>mm_pbsa.log') #execute the mm_pbsa calculation, this step may needs ~10mins
	#print "mmpbsa finished",time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time()))
